...
class LaporanEfisiensiService
{
    public static function getLaporan($startDate = null, $endDate = null): array
    {
        $supply = SuplayBarang::when(
            $startDate && $endDate,
            fn($q) => $q->whereBetween('tgl_pengiriman', [$startDate, $endDate])
        )->sum('jumlah');

        $keluar = BarangKeluar::when(
            $startDate && $endDate,
            fn($q) => $q->whereBetween('tgl_keluar', [$startDate, $endDate])
        )->sum('jumlah');
        ...
        $return = ReturnBarang::when(
            $startDate && $endDate,
            fn($q) => $q->whereBetween('tanggal_r', [$startDate, $endDate])
        )->sum('jumlah');

        $biaya = Pengeluaran::when(
            $startDate && $endDate,
            fn($q) => $q->whereBetween('tgl_pengeluaran', [$startDate, $endDate])
        )->sum('biaya');
